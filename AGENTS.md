...
Keep `public/sitemap.xml` as a static file generated by `scripts/generate-sitemap.mjs` (do not add `src/pages/sitemap.xml.ts` — a generated route has 500’d on Vercel). `vercel.json` deploys `dist` as a static output (`framework: null`) and sets `application/xml` on `/sitemap.xml`. Do not set `cleanUrls: true` — that 308s `google1dc06f863aa0062d.html`. List every public route, including `/robots.txt`. Canonical host is `https://www.privateandorra.com`. Keep `public/robots.txt` pointing at that sitemap. Keep `compressHTML: false` so inline links keep their neighbouring spaces.
